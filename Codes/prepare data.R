...
df[which(is.na(df$mental.icw.index)&df$year==2019),"mental.icw.index"] <-
  scale(df[which(is.na(df$mental.icw.index)&df$year==2019),]$mental)[,1]
...
# ## supplement 2011 and 2013 NA by the special question at the age of 29
# df$calm29 <- 7 - df$calm29
# pca <- princomp(df[which(df$year >= 2011 & df$year <= 2013 & is.na(df$mental.pc.index)),
#                    c("calm29", "blue29")][complete.cases(df[which(df$year >= 2011 & df$year <= 2013 & is.na(df$mental.pc.index)),
#                                                             c("calm29", "blue29")]), ],
#                 cor=TRUE, scores=TRUE)
# df[which(df$year >= 2011 & df$year <= 2013 & is.na(df$mental.pc.index)),
#    "mental.pc.index"][complete.cases(df[which(df$year >= 2011 &
#                                                 df$year <= 2013 &
#                                                 is.na(df$mental.pc.index)),
#                                         c("calm29", "blue29")]), ] <- pca$scores[, 1]
...
## here create lag for schedule control for 2019 
df <-
  df %>%
  group_by(ID) %>%
  complete(year = min(year):max(year)) %>% 
  mutate(lag.control = if_else(year - lag(year, 2) == 2, 
                               lag(control, 2), 
                               NA_real_)) %>%
  filter(year==2011|year==2013|year==2015|year==2017|year==2019)
df <- df %>% filter(!is.na(birth.year))

df[which(df$year==2019),"control"] <- df[which(df$year==2019),"lag.control"] 
df <- df %>% dplyr::select(-c("lag.control"))

## or I impute the missing by mean
df <- df %>%
  group_by(ID) %>%
  mutate(control = case_when(year==2019 ~ round(mean(control,na.rm=T)),
                             .default = control))
...
df <-
  ...
  mutate(lag.mental.pc.index = if_else(year - lag(year, 2) == 2, 
                               ...
                               NA_real_)) %>% 
  complete(year = min(year):max(year)) %>% 
  mutate(lag.mental.icw.index = if_else(year - lag(year, 2) == 2, 
                                       lag(mental.icw.index, 2), 
                                       NA_real_)) %>%
  filter(year==2011|year==2013|year==2015|year==2017|year==2019)
df <- df %>% filter(!is.na(birth.year))
...
df_st <-
  cbind(df$ID,df$year, df$sw, df$race,df$sex, df$industry, df$edu_cat, df$occupation,
        scale(df[,c("ghealth","mental.pc.index", "precarious","education",
                    ...
                    "urban","union","spouse.precarious", "mental.icw.index",
                    "injill","Lz","lag.ghealth","lag.precarious",
                    "lag.Lz","lag.mental.pc.index","lag.mental.icw.index",
...
write.csv(df_st,"/Users/wenhao/Dropbox/RA Linsey/NLSY97/precarious/Cleaned Data/master_01_trial.csv")
